...
def process(speaker, name):
    ...
    # load all train, test, valid dataset and concatenate them
    for name in ['train', 'test', 'valid']:
        dataset_path = os.path.join('dataset', speaker, f"{name}.json")
        dataset_ = load_dataset('json', data_files=dataset_path)
        if name == 'train':
            dataset = dataset_
        else:
            dataset['train'] = concatenate_datasets([dataset['train'], dataset_['train']])
...
model_path = f'VC-almost-all-09-33-15.52'
...
for speaker, revision in zip(speakers, revisions):
    ...
    test_ser, test_cer, test_wer_numerator, test_wer_no_ins_numerator, all_references = 0, 0, 0, 0, [0]
    end = time()
