...
def split_dataset(dataset_dict, test_size=300):
    ...
    test_indices.extend(remaining_indices[:test_size])
    print('test_indices', test_indices)
    # save the test indices in a file
    ...
    split = int(0.33 * len(train_indices))
...
repo_name = 'whisper-small-voice-conversion-korean-10min'
...
def compute_metrics_debug(pred, max_samples=10):
    ...
    print(f"Debugging WER and CER for {min(max_samples, len(predictions))} samples:")
    for i, (pred, ref) in enumerate(zip(predictions, references)):
        ...
        ref_words = ref.split()
        print(f"Prediction word count: {len(pred_words)}")
        print(f"Reference word count: {len(ref_words)}")

        pred_chars = list(pred)
        ref_chars = list(ref)
        print(f"Prediction char count: {len(pred_chars)}")
        print(f"Reference char count: {len(ref_chars)}")

        if wer > 100 or cer > 100:
            print("WARNING: WER or CER exceeds 100%. This indicates a potential issue.")
            if len(pred_words) == 0 and len(ref_words) > 0:
                print("The prediction is empty while the reference is not. This can cause inflated error rates.")
            elif len(pred_words) > 0 and len(ref_words) == 0:
                print("The reference is empty while the prediction is not. This can cause inflated error rates.")
