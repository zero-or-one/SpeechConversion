...
def load_folds(base_path, fold_num):
    atypical_voice = DatasetDict()
    
    # Load the specified fold as the test set
    test_path = f"{base_path}/fold{fold_num}.json"
    atypical_voice['test'] = load_dataset('json', data_files=test_path)['train']
    
    # Load all other folds as the train set
    train_datasets = []
    for i in range(5):  # Assuming 5 folds in total
        if i != fold_num:
            fold_path = f"{base_path}/fold{i}.json"
            train_datasets.append(load_dataset('json', data_files=fold_path)['train'])
    
    # Concatenate all train datasets
    atypical_voice['train'] = concatenate_datasets(train_datasets)
    
    return atypical_voice

# Usage
FOLD_NUM = 4  # This can be changed to any number from 0 to 4
print(f"Loading fold {FOLD_NUM}")
base_path = '/home/sabina/speech_handicap_dataset/imijeong/folds'  # Adjust this to your folds directory path

atypical_voice = load_folds(base_path, FOLD_NUM)

print(f"Train set size: {len(atypical_voice['train'])}")
print(f"Test set size: {len(atypical_voice['test'])}")
...
repo_name = f'VC-{train_duration_str}-fold{FOLD_NUM}'
...
training_args = Seq2SeqTrainingArguments(
    ...
    learning_rate=1e-5,
    num_train_epochs=15,  # Maximum number of epochs
